...
#Cambiar formato de fecha para filtrar datos
datos$Fecha<-as.Date(gsub("\\.","-",datos$Fecha))
#View(datos[which(datos$Fecha>="2017-01-01"),])
...
bands<-as.data.frame(PBands(datos[,6],n=20,maType="EMA",sd=2))
...
##### Prueba ifelse
datos1<-na.omit(cbind(datos,ret,ema.14,ema.30,rsi.14,macd))
ret<-as.numeric(datos1$ret)
datos1$dir<-ifelse(as.numeric(datos$ret)<=quantile(ret,.20),"Baja",ifelse(as.numeric(datos$ret)>=quantile(ret,.80),"Sube","Mantiene"))
#####

datos<-cbind(datos,ret,ema.14,ema.30,rsi.14,macd,"dir"=rep(0,length(datos[,1])),bands,wpr)
...
datos[,14]<-"Mantiene"
datos[which(as.numeric(datos$ret)<=quantile(ret,.20)),14]<-"Baja"
datos[which(as.numeric(datos$ret)>=quantile(ret,.80)),14]<-"Sube"
...
datospred<- datos[301:399,]
datos<-datos[1:300,]
#summary(datos)
#summary(datospred)
x<-sapply(datos, function(x) typeof(x))
View(x)
...
sum(diag(conf))/sum(conf)
View(as.data.frame(cbind(pred1,datospred$ret)))
